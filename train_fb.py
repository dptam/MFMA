...
class DataTrainingArguments:
    ...
    odd_even: Optional[str] = field(
        default='all', metadata={"help":"which split"}
    )
    mask_ratio: Optional[float] = field(
        default=0.6, metadata={"help":"source mask ratio"}
    )
    mask_type: Optional[str] = field(
        default='np', metadata={"help":"source mask type"}
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        if mask_ratio != 0.0:
            if data_args.mask_type == 'np':
                inputs_1 = [mask_input_np(x, ratio=mask_ratio_s) for x in inputs_1]
                inputs_2 = [mask_input_np(x, ratio=mask_ratio_a) for x in inputs_2]

            else:
                inputs_1 = [mask_input_split(x, ratio=mask_ratio_s) for x in inputs_1]
                inputs_2 = [mask_input_split(x, ratio=mask_ratio_a) for x in inputs_2]
        inputs = ['summary: '+x+' article: '+y for x,y in zip(inputs_1, inputs_2)]
        targets = examples[summary_column]
        targets = ['summary: '+x for x in targets]
    ...
    if training_args.do_train:
        ...
        if data_args.odd_even != 'all':
            if data_args.odd_even == 'even':
                subset = [x for i,x in enumerate(train_dataset) if i % 2 == 0]
            else:
                subset = [x for i,x in enumerate(train_dataset) if i % 2 == 1]

            subset_ = {}
            for key in train_dataset[0]:
                subset_[key] = [x[key] for x in subset]
            train_dataset = Dataset.from_dict(subset_)
